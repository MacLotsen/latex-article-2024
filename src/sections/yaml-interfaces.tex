...
Zoals eerder aangegeven, voor \LaTeX-gebruikers zou dit volledig weggelaten kunnen worden en direct gespecificeerd kunnen worden in \LaTeX\ zelf.
...
In hoofdstuk~\ref{sec:typesetting} is te zien hoe deze lijst onderaan de factuur wordt ingeladen.
...
Voor wat betreft de munteenheid heb ik ervoor gekozen om het in de macro \cs{currency} te vermommen.
...
\noindent
Waarden, ook wel de \textit{payload} genoemd, worden vergelijkbaar als recepten ingeladen, maar dan met de macro \cs{loadpayload}.
Dankzij de relaties, beschreven in hoofdstuk~\ref{sec:invoice data}, is het net wat complexer dan \textit{recepten}, omdat \pkg{lua-placeholders} hier niet iets standaard voor aanbied.
\lstinputlisting[language={[LaTeX]TeX},numbers=left,xleftmargin=15pt,firstnumber=51,linerange=51-54]{demo/invoice.tex}
Voor het inladen van de factuurwaarden wordt er namelijk gecontroleerd of er een corresponderend YAML-bestand voor bestaat.
...
In het geval dat er geen corresponderend bestand voor is gevonden, dan wordt er een factuursjabloon gecompileerd.
...
Voor dit soort listige situaties is de macro \cs{rawparam} geschreven, zoals gedaan voor de klant en leverancier.
...
Zoals te zien is, verschilt het inladen van de leverancier niet met het inladen van de klant.
...
Je kunt er ook voor kiezen om standaard alsnog de stijlwaarden te zetten aan de hand van de standaardwaarden opgegeven in het \texttt{style}-recept, door de configuratie daarvan buiten het \cs{hasparam} block te zetten.
...
Dit had ook gekund als er bijvoorbeeld een veld was \texttt{address lines} van het type \texttt{list}.
...
De algehele structuur van de tabel is nog afkomstig van de vorige situatie.
Het opmerkelijke verschil ten opzichte van de nieuwe situatie is dat de data in allerlei tabelstructuren gezet kan worden, aangezien de data los is gekoppeld van het \LaTeX- en applicatiedomein en de uitdagingen op het gebied van typesetting is verlegd naar het \LaTeX-domein.
